  ...
  for(const route of ['home','chat','tasks','notes','archive','people','budget','settings','reminders']){
   ...
   if(route==='chat'){
    ...
    const preservedAnchor=await page.evaluate(()=>new Promise(resolve=>{
     const before=document.querySelector('.chat');before.scrollTop=Math.min(180,before.scrollHeight);before.dispatchEvent(new Event('scroll'));
     requestAnimationFrame(()=>{const snapshot=window.NoemaChatScroll.snapshot(),old=before.querySelector(`[data-message-key="${snapshot.anchorKey}"]`)?.getBoundingClientRect().top-before.getBoundingClientRect().top;window.NoemaChatScrollRestore=snapshot;render();requestAnimationFrame(()=>requestAnimationFrame(()=>{const after=document.querySelector('.chat'),node=[...after.querySelectorAll('[data-message-key]')].find(item=>item.dataset.messageKey===snapshot.anchorKey);resolve({old,next:node?node.getBoundingClientRect().top-after.getBoundingClientRect().top:null,following:window.NoemaChatScroll.followingBottom})}))})
    }));
