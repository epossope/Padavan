...
class MiniAppSecurityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_chat_stream_is_ndjson_and_preserves_deltas(self):
        ...
        concatenated = "".join(event["text"] for event in events if event["type"] == "delta")
        completed = next(event["text"] for event in events if event["type"] == "done")
        self.assertEqual(concatenated, completed)
    ...
    def test_production_voice_is_global_batch_path_and_realtime_is_beta_only(self):
        ...
        self.assertIn("event.type==='speech_delta'", app_source)
        self.assertNotIn("function sanitizeSpeechText", app_source)
