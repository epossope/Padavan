...
function asset(request,response){const name=decodeURIComponent(new URL(request.url,'http://x').pathname.slice('/app/assets/'.length)),file=path.resolve(root,name);if(!file.startsWith(root)||!fs.existsSync(file)){response.writeHead(404);return response.end()}const type=name.endsWith('.js')?'application/javascript':name.endsWith('.css')?'text/css':name.endsWith('.json')?'application/json':'application/octet-stream';response.writeHead(200,{'Content-Type':type});fs.createReadStream(file).pipe(response)}
...
async function open(browser,base,modeName){
 ...
 await context.route('https://telegram.org/js/telegram-web-app.js',route=>route.fulfill({contentType:'application/javascript',body:`
  ...
  const mode=new URL(location.href).searchParams.get('mode');if(mode==='sdk-delayed')setTimeout(boot,250);else if(mode!=='sdk-missing')boot();
 `}));
 const page=await context.newPage();await page.goto(`${base}/app?mode=${modeName}`,{waitUntil:'domcontentloaded'});return {context,page};
}
(async()=>{
