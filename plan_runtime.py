...
class PlanValidator:
    ...
    def validate(self, trusted_owner: int, plan: SemanticPlan, *, conversation_context: dict | None, now: Any, timezone: str, request_id: str) -> ValidatedPlan:
        ...
        def resolve(ref: EntityReference, allow_create: bool) -> EntityReference:
            ...
            resolved, outcome = self.resolver.resolve_reference(trusted_owner, ref, conversation_context=conversation_context, allow_create=allow_create)
            if outcome.status == ResolutionStatus.AMBIGUOUS: raise PlanValidationError("ambiguous_person", "Уточни, о каком человеке речь.")
            if outcome.status == ResolutionStatus.NOT_FOUND: raise PlanValidationError("person_not_found", "Не удалось точно определить человека.")
        ...
        for index, item in enumerate(plan.actions):
            ...
            if _has_exact_id(item.fields): raise PlanValidationError("model_exact_id")
...
class BotDomainServices:
    ...
    def write(self, owner: int, item: ValidatedAction, dependency_results: dict[str, dict[str, Any]], timezone_name: str) -> dict[str, Any]:
        ...
        if item.domain == "person" and item.operation in {"upsert", "resolve_or_create"}: return {"ok": True, "id": people[0] if people else None}
        if item.domain == "event" and item.operation == "create":
            return self.bot.event_create(owner, fields.pop("title", "Встреча"), fields.pop("starts_at"), kind=fields.pop("kind", "other"), all_day=fields.pop("all_day", False), timezone=timezone_name, participant_ids=people, **fields)
        if item.domain == "reminder" and item.operation == "create":
            ...
            for dependency in item.depends_on:
                event_id = dependency_results.get(dependency, {}).get("id")
                if event_id and result.get("ok"):
                    with self.bot.conn() as connection: connection.execute("UPDATE reminders SET event_id=? WHERE id=? AND chat_id=?", (event_id, result["id"], owner))
            return result
        ...
        if item.domain == "event" and item.operation in {"delete", "cancel", "update"}:
            targets = [x for result in dependency_results.values() for x in result.get("events", [])]
...
class PlanExecutor:
    ...
    def execute(self, plan: ValidatedPlan, *, timezone_name: str) -> ExecutionResult:
        ...
        with self.connection_factory() as c:
            prior = c.execute("SELECT status,result_json FROM semantic_executions WHERE chat_id=? AND request_id=?", (plan.owner, plan.request_id)).fetchone()
            ...
            c.execute("INSERT OR IGNORE INTO semantic_executions(chat_id,request_id,plan_fingerprint,status,result_json,created_at) VALUES(?,?,?,?,?,?)", (plan.owner, plan.request_id, fingerprint, "VALIDATED", "", now))
        self._emit("semantic_execution_started"); result = ExecutionResult("EXECUTED", plan.request_id); deps: dict[str, dict[str, Any]] = {}
        try:
            for item in plan.reads:
                value = self.services.read(plan.owner, item); deps[item.read_id] = value; result.reads.append(ExecutionStep(item.read_id, "read", "EXECUTED", value))
            for item in plan.actions:
                value = self.services.write(plan.owner, item, deps, timezone_name); deps[item.action_id] = value
                ...
                if value.get("id") is not None: result.created_entities.append({"domain": item.domain, "id": value["id"]})
