...
const personGroups=['Семья','Друзья','Работа','Учёба','Проекты','Хобби','Путешествия / Спорт','Другое'];
const peopleCategory=person=>Array.isArray(person.groups)?person.groups.filter(group=>personGroups.includes(group)):['Другое'];
...
people=function(){
 const available=personGroups.filter(group=>data.people.some(person=>peopleCategory(person).includes(group))),primary=available.slice(0,4),overflow=available.slice(4);
 const found=data.people.filter(p=>`${p.name} ${p.notes} ${p.relationship} ${p.projects} ${(p.tags||[]).join(' ')}`.toLowerCase().includes(search.toLowerCase())&&(peopleFilter==='all'||peopleCategory(p).includes(peopleFilter)));
 return `${searchField('Поиск по людям…',search,'Поиск по людям')}<div class="screen-toolbar"><div class="chip-row">${chip('all','Все',peopleFilter==='all','people-filter')}${primary.map(value=>chip(value,value,peopleFilter===value,'people-filter')).join('')}${overflow.length?`<select class="people-overflow" aria-label="Другие группы" data-people-overflow><option value="">Ещё</option>${overflow.map(value=>`<option value="${esc(value)}" ${peopleFilter===value?'selected':''}>${esc(value)}</option>`).join('')}</select>`:''}</div><button class="secondary add-action" data-form="person">${icon('plus')}<span>Человек</span></button></div><div class="people-list">${found.map((p,index)=>{const facts=[p.relationship,p.home_city&&`из ${p.home_city}`,p.current_location&&`сейчас ${p.current_location}`,p.age&&`${p.age} лет`].filter(Boolean),projects=String(p.projects||'').split(/[,;]\s*/).filter(Boolean),tags=p.tags||[];return `<details class="card person-profile" ${index===0?'open':''}><summary><span class="avatar" data-avatar-file="${p.avatar_file_id||''}">${p.avatar_file_id?'<span class="avatar-loading"></span>':esc(p.name?.[0]||'•')}</span><div class="body"><h2>${esc(p.name)}</h2><small>${esc(facts.join(' · ')||'Контакт')}</small></div>${p.birthday?`<small class="person-birthday">${icon('calendar')}${esc(p.birthday)}</small>`:''}${icon('chevron')}</summary><div class="details"><div class="person-groups">${peopleCategory(p).map(group=>`<span>${esc(group)}</span>`).join('')}${tags.map(tag=>`<span class="tag">${esc(tag)}</span>`).join('')}</div><div class="person-overview"><div><h3>Что важно</h3><p>${esc(p.notes||'Пока нет заметок')}</p></div>${p.birthday?`<p><small>День рождения</small>${esc(p.birthday)}</p>`:''}</div>${p.recent_interactions?.length?`<h3>Недавние упоминания</h3><div class="profile-interactions">${p.recent_interactions.map(i=>`<p class="profile-interaction">${esc(i.interaction)}<small>${esc(i.interaction_date)}${i.interaction_type?` · ${esc(i.interaction_type)}`:''}</small></p>`).join('')}</div>`:''}${projects.length?`<h3>Связано с</h3><div class="project-chips">${projects.map(project=>`<span>${icon('folder')}${esc(project)}</span>`).join('')}</div>`:''}<div class="toolbar compact-actions"><label class="secondary avatar-action">${icon('image')}<span>${p.avatar_file_id?'Заменить фото':'Добавить фото'}</span><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" data-person-avatar="${p.id}"></label>${p.avatar_file_id?iconButton('trash','Удалить фото',`data-avatar-remove="${p.id}"`):''}${iconButton('settings','Изменить человека',`data-edit="person" data-id="${p.id}"`)}${iconButton('trash','Удалить человека',`data-delete="delete_person" data-id="${p.id}"`)}</div></div></details>`}).join('')||empty('Добавь человека — Noema будет хранить важный контекст.')}</div>`;
};
...
archive=function(){
 ...
 return `${searchField('Найти в архиве…',search,'Поиск по архиву')}<div class="screen-toolbar"><div class="chip-row">${chip('all','Все',filter==='all')}${kinds.map(kind=>chip(kind,resourceLabels[kind],filter===kind)).join('')}</div><button class="secondary add-action" data-form="note">${icon('plus')}<span>Заметка</span></button></div><div class="resource-list">${items.map(item=>`<article class="card resource-card" role="button" tabindex="0" data-archive-open="${item.type}:${item.id}"><div class="resource-thumb ${item.resourceKind}">${icon(resourceIcons[item.resourceKind])}</div><div class="resource-body"><h2>${esc(item.name?.slice(0,120)||'Без названия')}</h2><p>${esc(item.text||'Сохранено в Noema')}</p><small>${esc(item.label)} · ${date(item.created_at)}</small></div>${item.type==='notes'?`<div class="resource-actions">${iconButton('settings','Изменить заметку',`data-edit="note" data-id="${item.id}"`)}${iconButton('trash','Удалить заметку',`data-delete="delete_note" data-id="${item.id}"`)}</div>`:''}</article>`).join('')||empty('Архив пока пуст.')}</div><button class="secondary memory-search" data-memory-search>${icon('search')}<span>Поиск по всей памяти</span></button>${archiveResults?card('Результаты поиска',archiveResults.results.map(r=>`<details class="archive-entry"><summary>${esc(r.title||r.summary||'Запись')}</summary><p>${esc(r.content||r.summary||r.text)}</p></details>`).join('')||empty('Совпадений не найдено')):''}`;
};
let budgetPeriod='month',budgetMonth='',budgetData=null,budgetLoading=false,budgetCurrency='',budgetCurrencyOpen=false,budgetConversionCurrency='USD',budgetConversion=null,budgetConversionLoading=false,budgetConversionError='';
const conversionCurrencies=['RUB','USD','EUR','CNY'];
...
function distributionChart(entries,sum){let offset=0;const patterns=['dots','diag','cross','dash','solid'];const slices=entries.map(([name,value],i)=>{const length=sum?value/sum*100:0,slice=`<circle cx="60" cy="60" r="48" pathLength="100" fill="none" stroke="url(#budget-${patterns[i%patterns.length]})" stroke-width="10" stroke-dasharray="${Math.max(0,length-.6)} ${100-length+.6}" stroke-dashoffset="${-offset}"/>`;offset+=length;return slice}).join('');return `<svg viewBox="0 0 120 120" role="img" aria-label="Расходы по категориям"><defs><pattern id="budget-dots" width="4" height="4" patternUnits="userSpaceOnUse"><rect width="4" height="4" fill="#e4e4e2"/><circle cx="1" cy="1" r=".7" fill="#303033"/></pattern><pattern id="budget-diag" width="5" height="5" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="5" height="5" fill="#b8b8b6"/><path d="M0 1h5" stroke="#252527" stroke-width="1"/></pattern><pattern id="budget-cross" width="5" height="5" patternUnits="userSpaceOnUse"><rect width="5" height="5" fill="#888"/><path d="M0 0 5 5M5 0 0 5" stroke="#eee" stroke-width=".7"/></pattern><pattern id="budget-dash" width="5" height="5" patternUnits="userSpaceOnUse"><rect width="5" height="5" fill="#555"/><path d="M0 1h5M0 4h5" stroke="#eee" stroke-width="1"/></pattern><pattern id="budget-solid" width="4" height="4" patternUnits="userSpaceOnUse"><rect width="4" height="4" fill="#efefed"/></pattern></defs><circle cx="60" cy="60" r="48" fill="none" stroke="var(--border)" stroke-width="10"/>${slices}<text x="60" y="61" text-anchor="middle" fill="currentColor" font-size="8">${sum?esc(formatCurrency(sum,budgetCurrency||'RUB')):'Нет данных'}</text></svg>`}
function formatCurrency(value,currency,maximumFractionDigits=0){try{return new Intl.NumberFormat('ru-RU',{style:'currency',currency,maximumFractionDigits}).format(Number(value)||0)}catch{return `${Number(value||0).toLocaleString('ru-RU')} ${esc(currency)}`}}
function currencyWidget(currencies,currency,total){
 const targets=[...new Set([...conversionCurrencies,...currencies])].filter(code=>code!==currency);
 if(!targets.includes(budgetConversionCurrency))budgetConversionCurrency=targets[0]||currency;
 const converted=budgetConversion&&budgetConversion.base===currency&&budgetConversion.quote===budgetConversionCurrency&&total?total.balance*budgetConversion.rate:null;
 const status=budgetConversionLoading?'Обновляю курс…':converted!==null?`1 ${esc(currency)} = ${Number(budgetConversion.rate).toLocaleString('ru-RU',{maximumFractionDigits:4})} ${esc(budgetConversionCurrency)} · Баланс ${formatCurrency(converted,budgetConversionCurrency,2)}`:budgetConversionError|| (preview?'Live-курс доступен при открытии через Telegram.':'Выбери валюту для пересчёта.');
 return `<div class="currency-widget"><button class="currency-widget-summary" type="button" data-currency-toggle aria-expanded="${budgetCurrencyOpen}"><span>${icon('globe')}<span><small>Валюта</small><b>${esc(currency)} → ${esc(budgetConversionCurrency)}</b></span></span>${icon('chevron')}</button>${budgetCurrencyOpen?`<div class="currency-widget-panel">${currencies.length>1?`<div class="currency-options source-options" aria-label="Валюта баланса">${currencies.map(code=>`<button type="button" class="currency-option ${code===currency?'selected':''}" data-budget-currency="${esc(code)}">${esc(code)}</button>`).join('')}</div>`:''}<div class="currency-options" aria-label="Валюта конвертации">${targets.map(code=>`<button type="button" class="currency-option ${code===budgetConversionCurrency?'selected':''}" data-convert-currency="${esc(code)}">${esc(code)}</button>`).join('')}</div><small class="currency-conversion" aria-live="polite">${status}</small></div>`:''}</div>`;
}
budget=function(){
 const stats=budgetData?.currency_totals||{},items=budgetData?.items||[],currencies=Object.keys(stats);if(!budgetCurrency||!stats[budgetCurrency])budgetCurrency=stats.RUB?'RUB':currencies[0]||'RUB';const currency=budgetCurrency,total=stats[currency];
 ...
 return `<div class="budget-controls"><label class="period-picker" for="budget-month">${icon('calendar')}<span>${esc(monthLabel)}</span><input type="month" id="budget-month" value="${budgetMonth||data.day.slice(0,7)}"></label>${segmented([['day','День'],['week','Неделя'],['month','Месяц'],['year','Год']],budgetPeriod,'budget-period')}</div><div class="screen-toolbar budget-actions"><div></div><div><button class="secondary add-action" data-form="expense">${icon('plus')}<span>Расход</span></button><button class="secondary add-action" data-form="income">${icon('plus')}<span>Доход</span></button></div></div><div class="budget-totals"><section class="card"><h3>Расходы</h3>${budgetLoading?'<div class="skeleton"></div>':`<p class="amount">${total?formatCurrency(total.expense,currency):'—'}</p><small>За выбранный период · ${esc(currency)}</small>`}<span class="orb-fragment"></span></section><section class="card"><h3>Доходы</h3>${budgetLoading?'<div class="skeleton"></div>':`<p class="amount">${total?formatCurrency(total.income,currency):'—'}</p><small>За выбранный период · ${esc(currency)}</small>`}<span class="orb-fragment"></span></section></div><div class="budget-dashboard"><section class="card distribution"><h3>Распределение расходов</h3><div class="distribution-body">${distributionChart(categoryEntries,categorySum)}<div class="expense-legend">${categoryEntries.map(([name,value],i)=>`<p><i class="pattern-chip ${['dots','diag','cross','dash','solid'][i%5]}"></i><b>${esc(name)}</b><span>${Math.round(value/categorySum*100)}%</span><small>${formatCurrency(value,currency)}</small></p>`).join('')||'<p class="subtle">После первого расхода здесь появятся категории.</p>'}</div></div></section><section class="card trend"><h3>Расходы по дням · ${esc(currency)}</h3>${values.length?`<div class="bars" role="img" aria-label="Расходы по дням">${values.map(([day,value])=>`<div class="bar-group" title="${esc(day)}: ${formatCurrency(value,currency)}"><i style="height:${Math.max(8,value/max*100)}%"></i><small>${day.slice(8)}</small></div>`).join('')}</div><small>Динамика выбранного периода</small>`:empty('Данных для графика пока нет.')}</section></div>${currencies.length?`<section class="card finance-strip">${currencyWidget(currencies,currency,total)}<div class="period-balance"><h3>Баланс периода</h3><p class="amount">${total?formatCurrency(total.balance,currency):'—'}</p></div></section>`:''}<section class="card operations"><div class="card-head"><h3>Последние операции</h3><span>${items.length}</span></div>${items.slice(0,100).map(e=>`<article class="operation-row"><span class="operation-icon">${icon(e.kind==='income'?'wallet':'card')}</span><div class="body"><p>${esc(e.description||'Операция')}</p><small>${esc(e.category||'Без категории')}</small></div><span class="mono">${e.kind==='income'?'+':'−'}${Number(e.amount).toLocaleString('ru-RU')} ${esc(e.currency)}</span><small>${date(e.spent_at)}</small>${iconButton('settings','Изменить операцию',`data-edit="expense" data-id="${e.id}"`)}${iconButton('trash','Удалить операцию',`data-delete="delete_expense" data-id="${e.id}"`)}</article>`).join('')||empty('Операций за период нет.')}</section>${items.length>100?'<p class="subtle">Показаны первые 100 операций. Сузь период для просмотра остальных.</p>':''}`;
};
function renderDock(){
 ...
 if(!dock.querySelector('canvas'))dock.innerHTML=`<button class="focus-pill" aria-label="Открыть фокус дня"><span></span></button><button class="orb-button" data-voice aria-label="Нажми для голосового запроса"><canvas class="membrane" width="260" height="260" aria-hidden="true"></canvas></button>${iconButton('chat','Открыть текстовый чат','data-keyboard')}`;
...
render=function(){
 ...
 renderDock();window.NoemaVoiceController?.sync?.();if(page==='people')requestAnimationFrame(()=>hydrateAvatarImages());if(page==='chat'){const restore=window.NoemaChatScrollRestore||null;window.NoemaChatScrollRestore=null;window.NoemaChatScroll?.bind?.(restore)}else window.NoemaChatScroll?.detach?.();syncSystemBackButton()
};
...
document.addEventListener('click',async e=>{
 ...
 if(b.hasAttribute('data-chat-latest'))window.NoemaChatScroll?.scrollToBottom(false);
 if(b.hasAttribute('data-voice'))await recordVoice(b);
 if(b.hasAttribute('data-layout'))openLayout();
 if(b.hasAttribute('data-close-layout')){document.querySelector('#layout-editor').close();syncSystemBackButton()}
 if(b.hasAttribute('data-reset-layout')){layoutDraft=[...defaultWidgets];drawLayout()}
 if(b.hasAttribute('data-save-layout')){if(preview){data.settings.home_widgets=[...layoutDraft];render()}else await mutate('home_layout',{widgets:layoutDraft});if(preview||!notice.textContent){document.querySelector('#layout-editor').close();syncSystemBackButton()}}
...
async function loadBudgetConversion(){
 if(!budgetCurrencyOpen)return;
 if(preview){budgetConversion=null;render();return}
 budgetConversionLoading=true;render();
 try{budgetConversionError='';budgetConversion=await api('exchange_rate',{base:budgetCurrency,quote:budgetConversionCurrency})}
 catch{budgetConversion=null;budgetConversionError='Курс временно недоступен.'}
