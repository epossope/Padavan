...
people=function(){const found=data.people.filter(p=>`${p.name} ${p.notes} ${p.relationship} ${p.projects} ${p.home_city} ${p.current_location}`.toLowerCase().includes(search.toLowerCase()));return `<input id="search" class="search" aria-label="Поиск по людям" placeholder="Поиск по людям…" value="${esc(search)}"><div class="toolbar"><button class="secondary" data-form="person">${icon('plus')}<span>Человек</span></button></div>${found.map(p=>{const facts=[p.relationship,p.home_city&&`из ${p.home_city}`,p.current_location&&`сейчас ${p.current_location}`,p.age&&`${p.age} лет`].filter(Boolean);return `<details class="card person-profile"><summary class="row"><span class="avatar">${esc(p.name?.[0]||'•')}</span><div class="body"><h2>${esc(p.name)}</h2><small>${esc(facts.join(' · ')||'Контакт')}</small></div><small>${p.recent_interactions?.length||0} упомин.</small></summary><div class="details"><div class="profile-facts">${p.birthday?`<p><small>День рождения</small>${esc(p.birthday)}</p>`:''}${p.projects?`<p><small>Связано с</small>${esc(p.projects)}</p>`:''}${p.notes?`<p><small>Что важно</small>${esc(p.notes)}</p>`:''}</div><h3>Недавние упоминания</h3>${p.recent_interactions?.map(i=>`<p class="profile-interaction">${esc(i.interaction)}<small>${esc(i.interaction_date)}${i.interaction_type?` · ${esc(i.interaction_type)}`:''}</small></p>`).join('')||'<p class="subtle">Пока нет упоминаний</p>'}<div class="toolbar compact-actions"><button class="secondary" data-edit="person" data-id="${p.id}">Изменить</button><button class="secondary" data-delete="delete_person" data-id="${p.id}">Удалить</button></div></div></details>`}).join('')||empty('Добавь человека — Noema будет хранить важный контекст.')}`;};
