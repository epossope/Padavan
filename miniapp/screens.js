...
tasks=function(){
 ...
 return `${segmented([['today','Сегодня'],['upcoming','Предстоящие'],['done','Завершённые']],taskTab,'task-tab')}<div class="screen-toolbar"><div class="chip-row">${filters}</div><button class="secondary add-action" data-form="task">${icon('plus')}<span>Задача</span></button></div>${sectionHeader(taskTab==='done'?'Завершённые':taskTab==='upcoming'?'Предстоящие':'Сегодня',`${items.length} ${items.length===1?'задача':'задач'}`)}<div class="task-list">${items.map(t=>`<article class="card task-card"><button class="check ${t.status!=='open'?'done':''}" data-toggle="${t.id}" aria-label="Изменить статус задачи"></button><details class="body"><summary class="task-summary"><div><h2>${esc(t.text)}</h2><small><i></i>${esc(priorityLabel(t.priority))}</small></div><div class="task-meta"><span>△ ${esc(priorityLabel(t.priority))}</span><small>${icon('clock')}${itemDate(t.due_date)}</small></div>${icon('chevron')}</summary><div class="details"><div class="detail-grid"><p><small>Срок</small>${itemDate(t.due_date)}</p><p><small>Приоритет</small>${esc(priorityLabel(t.priority))}</p></div><div class="toolbar compact-actions"><button class="secondary" data-edit="task" data-id="${t.id}">Изменить</button><button class="secondary" data-delete="delete_task" data-id="${t.id}">Удалить</button></div></div></details></article>`).join('')||empty('В этом разделе нет задач')}</div>`;
};
