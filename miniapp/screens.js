...
render=function(){
 ...
 renderDock();window.NoemaVoiceController?.sync?.();if(page==='people')requestAnimationFrame(()=>hydrateAvatarImages());if(page==='chat'){const restore=window.NoemaChatScrollRestore||null;window.NoemaChatScrollRestore=null;window.NoemaChatScroll?.bind?.(restore)}else window.NoemaChatScroll?.detach?.();tg?.BackButton.hide()
};
...
function back(){if(document.querySelector('#archive-preview')?.open){document.querySelector('#archive-preview').close();return}if(document.querySelector('#layout-editor').open){document.querySelector('#layout-editor').close();return}if(document.querySelector('#editor').open){document.querySelector('#editor').close();return}page=navHistory.pop()||'home';search='';filter='all';taskFilter='all';noteFilter='all';peopleFilter='all';render()}
