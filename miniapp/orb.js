...
(() => {
  ...
  function draw(ms){
    ...
    for(const canvas of document.querySelectorAll('canvas.membrane')){
      ...
      const c=canvas.getContext('2d'), w=canvas.width, r=w*.37*(1+Math.sin(t)*.012+breathing+(state==='listening'?amplitude*.04:0)+pulse*.03), mid=w/2;
      if(!c)continue;c.globalAlpha=opacity;
      c.clearRect(0,0,w,w);
      const halo=c.createRadialGradient(mid,mid,r*.4,mid,mid,r*1.32);
      halo.addColorStop(0,'#f5f5f503');halo.addColorStop(.5,state==='idle'?'#f5f5f522':state==='listening'?'#ffffff3d':'#f8f8f835');halo.addColorStop(1,'#f5f5f500');c.fillStyle=halo;c.fillRect(0,0,w,w);
      ...
      for(let layer=0;layer<5;layer++){
        ...
        for(let i=0;i<=220;i++){
          ...
          const rad=r*(1+ripple+Math.sin(layer*1.7+t)*.065);
          const x=mid+Math.cos(a)*rad, y=mid+Math.sin(a)*rad*(.985+Math.sin(t*.4+layer)*.015);
          i?c.lineTo(x,y):c.moveTo(x,y);
        }
        c.closePath();
        const surface=c.createRadialGradient(mid-r*.6,mid-r*.7,r*.05,mid,mid,r*1.05);
        surface.addColorStop(0,'#ffffff04');surface.addColorStop(.38,'#c8c9cc02');surface.addColorStop(.78,'#05060600');surface.addColorStop(.93,'#ffffff08');surface.addColorStop(1,'#ffffff1c');
        ...
        c.strokeStyle=rim;c.lineWidth=w/390;c.shadowColor='#ffffff';c.shadowBlur=layer===0?18:7;c.stroke();c.shadowBlur=0;
      }
      ...
      for(let fold=0;fold<5;fold++){
        ...
        const sheen=c.createLinearGradient(-r,0,r,0);sheen.addColorStop(0,'#f5f5f500');sheen.addColorStop(.22,'#ffffff48');sheen.addColorStop(.5,'#ffffff0a');sheen.addColorStop(.82,'#ffffffad');sheen.addColorStop(1,'#f5f5f500');
