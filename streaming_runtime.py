...
class ToolPackResolver:
    ...
    def required_tool_choice(self, text: str):
        ...
        """
        lowered = str(text or "").lower()
        task_words = ("какие у меня задачи", "что нужно сделать сегодня", "что просрочено", "покажи завершённые", "есть задача")
        ...
        note_words = ("покажи мои заметки", "найди заметку", "что я записывал", "что я записывала")
        if any(word in lowered for word in note_words):
            return {"type": "function", "function": {"name": "note_list"}}
