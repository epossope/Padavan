...
def register_miniapp(app, core):
    ...
    async def api(request):
        try:
            ...
            if action == "state":
                ...
                result = {"ok": True}
            elif action in {"add_task", "delete_task", "save_note", "delete_note", "set_reminder", "delete_reminder",
                            "add_expense", "add_income", "delete_expense", "person_upsert", "delete_person",
    ...
    def state(cid, args):
        ...
        with core.conn() as c:
            tasks = [dict(r) for r in c.execute("SELECT id,text,due_date,status FROM tasks WHERE chat_id=? ORDER BY status,due_date,id DESC LIMIT 200", (cid,))]
