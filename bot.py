...
async def callback(update,context):
    ...
    if q.data in ("iphone:add", "iphone:add:force"):
        ...
        text = (
            ...
            '<b>Ключ устройства</b> — нажмите на строку, чтобы скопировать:\n'
            f'<code>{device["secret"]}</code>\n\n'
            'Дальше на iPhone: откройте <b>Команды (Shortcuts)</b> → <b>+</b> → «Добавить действие» → «Получить содержимое URL». '
            'Для заполнения команды нажмите «Параметры Shortcut» ниже. Затем в настройках iPhone назначьте созданную команду на Action Button или Back Tap.'
        )
    ...
    if q.data.startswith("iphone:shortcut:"):
        ...
        text = (
            '<b>Параметры для одной команды Shortcut</b>\n\n'
            'В «Получить содержимое URL» выберите метод <b>POST</b> и JSON. Вставьте:\n'
            f'Адрес: <code>{html.escape(endpoint)}</code>\n'
            f'ID устройства: <code>{device_id}</code>\n\n'
            'В JSON добавьте поля <code>device_id</code>, <code>secret</code> и <code>trigger</code>. '
            'Значение <code>trigger</code>: <code>action</code>, <code>double</code> или <code>triple</code>. '
            'Для голосовой заметки добавьте поле <code>text</code> из действия «Продиктовать текст».\n\n'
            'Ключ устройства показан в сообщении о подключении. Не пересылайте его другим.'
        )
        return await q.edit_message_text(text, parse_mode="HTML",
