...
BUILD_ID = "preview-ui-2026-09-09.3"
...
def create_quick_action_device(chat_id, name="iPhone"):
    ...
    with conn() as c:
        c.execute("INSERT INTO quick_action_devices(id,chat_id,name,secret_hash,created_at) VALUES(?,?,?,?,?)",
...
def quick_action_devices(chat_id):
    with conn() as c:
        devices = [dict(row) for row in c.execute("SELECT id,name,active,last_used_at FROM quick_action_devices WHERE chat_id=? ORDER BY created_at DESC", (chat_id,)).fetchall()]
...
def revoke_quick_action_device(chat_id, device_id):
    with conn() as c:
        cur = c.execute("UPDATE quick_action_devices SET active=0 WHERE id=? AND chat_id=?", (device_id, chat_id))
...
def usage_summary(chat_id=None, days=30):
...
async def callback(update,context):
    ...
    if q.data == "iphone:add":
        ...
        text = (
            ...
            'Сохраните эти значения в команде Shortcuts. Ключ показывается только сейчас.\n'
            f'URL: <code>{html.escape(endpoint)}</code>\n'
            f'ID устройства: <code>{device["id"]}</code>\n'
            f'Ключ устройства: <code>{device["secret"]}</code>\n\n'
            'Следующим шагом выберите действия для Action Button, двойного и тройного касания.'
        )
        return await q.edit_message_text(text, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Настроить действия", callback_data=f'iphone:device:{device["id"]}')],
    ...
    if q.data == "keys:admin_usage":
        ...
        text = usage_text(usage_summary(), "📈 <b>Общий ключ: расход за 30 дней</b>")
        return await q.edit_message_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("‹ API-ключи", callback_data="settings:keys")]]))
...
def settings_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📜 Правила", callback_data="settings:rules"), InlineKeyboardButton("📱 iPhone", callback_data="settings:iphone")],
...
def iphone_device_page(chat_id, device_id):
    ...
    buttons += [[InlineKeyboardButton("🗑 Отключить iPhone", callback_data=f'iphone:revokeask:{device_id}')],
...
def usage_text(rows, title):
    ...
    for row in rows[:8]:
        ...
        lines.append(f'<code>{html.escape(row["model"])}</code> · {source} · {row["requests"]} запр. · ${float(row["cost"] or 0):.4f}')
    return "\n".join(lines)
...
async def quick_actions_run(request):
    ...
    if not device_id or not secret or trigger not in ("action", "double", "triple"):
    ...
    try:
        await request.app["telegram_app"].bot.send_message(chat_id=device["chat_id"], text=message)
    except Exception:
