...
async def callback(update,context):
    ...
    if q.data in ("iphone:add", "iphone:add:force"):
        ...
        text = (
            ...
            '<b>ID устройства</b> — нажмите, чтобы скопировать:\n'
            f'<code>{device["id"]}</code>\n\n'
            '<b>Ключ устройства</b> — нажмите, чтобы скопировать:\n'
    ...
    if q.data.startswith("iphone:shortcut:"):
        ...
        secret_line = f'<code>{secret}</code>' if secret else 'Нет сохранённого ключа у старого подключения. Выпустите новый ключ ниже.'
        text = (
            '<b>Параметры для одной команды Shortcut</b>\n\n'
            '1. В «Команды» нажмите <b>+</b> → добавьте действие <b>URL</b> и вставьте адрес.\n'
            f'<code>{html.escape(endpoint)}</code>\n\n'
            '2. Добавьте действие <b>Получить содержимое URL</b> → «Показать больше» → метод <b>POST</b> → тело запроса <b>JSON</b>.\n\n'
            '3. Добавьте в JSON три текстовых поля:\n'
            f'<code>device_id</code> = <code>{device_id}</code>\n'
            f'<code>secret</code> = {secret_line}\n'
            '<code>trigger</code> = <code>action</code> / <code>double</code> / <code>triple</code>\n\n'
            'Для голосовой заметки перед URL добавьте «Продиктовать текст» и передайте его результат в поле <code>text</code>. '
            'Для первой проверки проще назначить действие «Что осталось сегодня»: ему текст не нужен.'
        )
        return await q.edit_message_text(text, parse_mode="HTML",
    ...
    if q.data.startswith("iphone:rotate:"):
        ...
        return await q.edit_message_text(f"🔑 <b>Новый ключ устройства</b>\nНажмите на строку, чтобы скопировать:\n<code>{secret}</code>\n\nЗамените только значение <code>secret</code> во всех командах Shortcut.",
